import { verifyEIP712Message, verifyMessage } from '../src';
...
describe('evm-chain', () => {
  ...
  test('should verify message and get address', async () => {
    const verified = await verifyMessage(testData.publicKey as Address, testData.message, testData.signature as Hex);
  ...
  test('should throw error if data invalid', async () => {
    await expect(verifyMessage('0xpublic', testData.message, testData.signature as Hex)).rejects.toThrow();
    await expect(verifyMessage(testData.publicKey as Address, testData.message, '11111' as any)).rejects.toThrow();
    await expect(verifyMessage(testData.publicKey as Address, 'xxxx', testData.signature as Hex)).resolves.toBe(false);
  });

  test('should verify EIP712 message and get address', async () => {
    const verified = await verifyEIP712Message(
    ...
    const actualAddress = getAddress(testDataForEIP712.publicKey);
    expect(actualAddress).toBe(testDataForEIP712.address);
  });

  test('should throw error if EIP712 data invalid', async () => {
    await expect(
      verifyEIP712Message(testDataForEIP712.publicKey as Address, 'abcde', testDataForEIP712.signature as Hex),
