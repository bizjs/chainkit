...
import { getAddress, getNativeAddress, verifyBIP322Message, verifyEcdsaMessage, verifyUnisatMessage } from '../src';
...
describe('bitcoin-chain', () => {
  ...
  test.skip('should verify bip322 message and get address', async () => {
    const verified = await verifyBIP322Message(
    ...
    let actualAddress = await getAddress(testDataForBip322.publicKey, 'segwit_native');
    expect(actualAddress).toBe(testDataForBip322.nativeAddress);

    actualAddress = await getAddress(testDataForBip322.publicKey, 'segwit_taproot');
    expect(actualAddress).toBe(testDataForBip322.taprootAddress);

    actualAddress = getNativeAddress(testDataForBip322.publicKey);
    expect(actualAddress).toBe(testDataForBip322.nativeAddress);
  });

  test('should throw error when get address with invalid address type', () => {
    expect(() => getAddress(testDataForBip322.publicKey, 'invalid' as any)).toThrow('Invalid addressType');
  });

  test('should verify unisat message and get data', async () => {
    const verified = await verifyUnisatMessage(
  ...
  test('should verify ecdsa message and get address', async () => {
    const verified = await verifyEcdsaMessage(
    ...
    const actualAddress = await getAddress(testDataForEcdsa.publicKey, 'segwit_taproot');
